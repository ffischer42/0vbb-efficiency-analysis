...
for c in 0:1:36 # change to 0:1:36 for ALL channels
    files = glob("pulses/sim/raw/"*string(c)*"-"*channel_to_name[c]*"/*-wf*.h5");
    if length(files) > 0
        push!(channels, c)
    end
end
    ...
    for file in files
        ...
        if !isfile(filename)
            ...
            for i in 1:1:length(evtno)
                temp = data_raw |> @select(:waveform, :chnid, :evtno, :event_mult, :detno, :hits_totnum, :edep, :pos) |> @filter(_.evtno == evtno[i]) |> Table
            ...
            #     wf_positive = [temp[a].waveform]
            #     wf_negative = [temp[b].waveform]
            ...
            #                 wf_negative = ArrayOfRDWaveforms( Array{typeof(t.wf_negative[1]), 1}(t.wf_negative) ),
            #                 wf_positive = ArrayOfRDWaveforms( Array{typeof(t.wf_positive[1]), 1}(t.wf_positive) ),
                            evtno       = Int32.(t.evtno),
