...
#     IJulia.clear_output(true)
    ...
    @info(">---------------------------------------------------------------<")
    files = glob(joinpath(base_path_raw, str_ch * "-" * channel_to_name[ch] * "/w_filter/*/*.h5"));
    number_of_pulses = 0;
    key_str = []
    for file in files
    ...
    @showprogress 1 "Counting simulated waveforms..." for k in key_str
        last_file = basename(glob(joinpath(base_path_raw, str_ch * "-" * channel_to_name[ch] * "/w_filter/*/" * String(k) * "*.h5"))[end])
