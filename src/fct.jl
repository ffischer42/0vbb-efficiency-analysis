...
simdir_old      = "/remote/ceph2/group/gerda/data/simulation/gerda-mage-sim"
        ...
        for set in keys(fit_params[det])
            for E in keys(fit_params[det][set]) 
                if "bat_params" in keys(fit_params[det][set][E])
                    append!(tt, Table(
                            detector=[det], set=[set], energy=[parse(Int64, E)], 
...
function movingaverage(X::Vector,numofele::Int,BackDelta::Int64,ForwardDelta::Int64)
    len = length(X)
    Y = similar(X)
    for n = 1:len
        lo = max(1,n - BackDelta)
...
function movingaverage(X::Vector,numofele::Int,BackDelta::Int64,ForwardDelta::Int64,iterations::Int)
    i = 1
    while i <= iterations
        X = movingaverage(X,numofele,BackDelta,ForwardDelta) 
        i += 1
    end
    return X    
end
...
function mwa(samples::Vector, window::Integer, reps::Integer)
    for i in 1:1:reps
        samples = mwa(samples, window)
    end
    return samples
